...
do
	...
	do
		for num_trial in $((4)) # number of actions in each transaction
		do
			...
			do
				...
				do
					...
					do
						for lambda_t in 0.01 0.02 0.025 0.030.035 0.04 # the lambda parameter for the possion distribution of transaction's arrival rate 
						do
							...
							do
								...
								do
									if [ "$use_power_law" = "true" ]; then
										...
										echo ${filename}
										./gen $num_obj $num_trans $num_trial $lambda_t $lambda_a $use_power_law $lock_type_prob $power_law_param >> testcase/${filename}
									else
